...
Write-Host "OneDrive Light Mode tray icons deployment is starting..." -ForegroundColor Green
...
ForEach ($searchDir in $searchDirList){
    ...
    if ($dllPath) {
        Write-Host "`nFound $dllPath" -ForegroundColor DarkYellow
...
Start-Sleep 1
Write-Host "`nOneDrive Light Mode tray icons have been applied. Please restart OneDrive." -ForegroundColor Cyan